...
public class Backtracking_Queen2023 {
	public static void solveQueen(int[][] d) {
		...
		while (true) {
			...
			if (count == 8) { //8개를 모두 배치하면
				System.out.println("퀸이 모두 배치됐습니다.");
			}
	...
	public static boolean checkHang(int[][] d, int crow) { //배열 d에서 행 row에 퀸을 배치할 수 있는지 조사
			for(int i =0; i<d.length; i++) {
				if(crow == i)		//퀸과 같은 행에 다른 퀸을 배치할 수 x / crow = 배치하려는 행 i는 검사하려는 행
					return false;	//퀸과 같은 행에 있으면 안되니까 fasle 반환.
			}
		
		return true;
	}

	public static boolean checkYeoul(int[][] d, int ccol) {//배열 d에서 열 col에 퀸을 배치할 수 있는지 조사
			for(int i =0; i<d[0].length; i++) {
				if(ccol ==i)		//퀸과 같은 열에 다른 퀸을 배치할 수 x / crow = 배치하려는 열 i는 검사하려는 열
					return false;
	}
			return true;
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < data.length; i++)	//현재 열에서 모든 열들을 조사 
			for (int j = 0; j < data[0].length; j++)	//현재 행에서 모든 행들을 조사 
				data[i][j] = 0;
